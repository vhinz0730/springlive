<div x-cloak x-show="open" @click.outside="open = false" class="w-full fixed z-50 top-0 right-0 h-full overflow-x-hidden transform translate-x-0 transition ease-in-out duration-700" id="notification">
    ...
    <div class="2xl:w-1/4 bg-gray-500 shadow-md mt-10 overflow-y-auto p-8 pt-3 absolute right-0 z-30">
        ...
        @if ($unread->count() > 0)
            <h2 tabindex="0" class="focus:outline-none text-sm leading-normal pt-8 border-b pb-2 border-gray-300 text-black">
                Unread Notifications
            </h2>

            @foreach ($unread as $announcement)
                <div class="w-full p-3 mt-4 text-black bg-white rounded flex flex-shrink-0 {{ $announcement->read_at === null ? "drop-shadow shadow border" : ""  }}">
                    <x-megaphone::display :notification="$announcement"></x-megaphone::display>

                    @if($announcement->read_at === null)
                        <button role="button" aria-label="Mark as Read" class="focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 rounded-md cursor-pointer"
        ...
        @foreach ($announcements as $announcement)
            @if ($announcement->created_at->diffInDays(now()) <= 30)
